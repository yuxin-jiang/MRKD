...
class SelfSupMVTecDataset(Dataset):
    def __init__(self, root_path='../data', class_name='bottle', is_train=True,
        ...
        self.mask1_transform = T.Compose([
            T.Resize((64, 64)),
            T.ToTensor()])
        self.mask2_transform = T.Compose([
            T.Resize((32, 32)),
            T.ToTensor()])
        self.mask3_transform = T.Compose([
            T.Resize((16, 16)),
            T.ToTensor()])
    ...
    def __getitem__(self, idx):
        ...
        if self.self_sup:
            ...
            mask_2 = self.mask2_transform(mask)
            mask_3 = self.mask3_transform(mask)
            if src_object_mask is not None:
                src_object_mask = torch.tensor(src_object_mask[None, ..., 0]).float().squeeze(dim=0)
            else:
                # mask_1 = 1
                # mask_2 = 1
                # mask_3 = 1
                src_object_mask = 1
                # mask_1 = torch.tensor(mask_1[None, ..., 0]).float().clone().detach().requires_grad_(True)
            # mask_2 = torch.tensor(mask_2[None, ..., 0]).float().clone().detach().requires_grad_(True)
            # mask_3 = torch.tensor(mask_3[None, ..., 0]).float().clone().detach().requires_grad_(True)
            self.prev_idx = idx
        else:
            if self.transform is not None:
        ...
        return image, x, y, mask1, src_object_mask, mask0, mask_1, mask_2, mask_3
            # , src_object_mask1, src_object_mask2, src_object_mask3
