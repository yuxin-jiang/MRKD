...
# def conv0x0(in_planes: int, out_planes: int, stride: int = 2) -> nn.Conv2d:
#     """0x0 convolution"""
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        feature_a = self.layer1(x)#[8, 256, 64, 64]
        feature_b = self.layer2(feature_a)#[8, 512, 32, 32]
        feature_c = self.layer3(feature_b)#[8, 1024, 16, 16]
        # feature_d = self.layer4(feature_c)#[8, 2048. 8, 8]
        ...
        # x_c = self.fc(x_c)
        # return feature_d
        # return [feature_a, feature_b, feature_c, x_a, x_b, x_c]
...
class BN_layer(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        #x = self.cbam(x)
        # x[0] = self.generation3(x[0])
        # x[1] = self.generation2(x[1])
        # x[2] = self.generation1(x[2])
        l1 = self.relu(self.bn2(self.conv2(self.relu(self.bn1(self.conv1(x[0]))))))
        l2 = self.relu(self.bn3(self.conv3(x[1])))
        feature = torch.cat([l1,l2,x[2]],1)
        # B, C, H, W = feature.size()
        # masked_img = feature.clone()
        # embedding_vectors = masked_img.view(B, C, H * W)
        ...
        # _, l = np.max(dist_list, axis=0)
        # for i in range(len(l)):
        #     h, w = l[i]
        #     feature[i, :, h-4:h+4, w-4:h+4] = 0

        output = self.bn_layer(feature)
        # N, C, H, W = output.shape
        # mat = torch.rand((N, 1, H, W)).cuda()
        # lambda_mgd = 0.05
        # mat = torch.where(mat > 1 - lambda_mgd, 0, 1).cuda()
        #
        # masked_fea = torch.mul(output, mat)
